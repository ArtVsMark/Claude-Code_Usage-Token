...
def test_литерал_не_по_схеме_находится(репозиторий: Path) -> None:
    """Номер, которого схема не знает, кончается выпуском не под тем именем."""
    ...
    assert len(беды) == 1
    assert "0.1.5" in беды[0] and "v0.1.0" in беды[0]
...
def test_выпущенная_версия_читается_из_одного_места() -> None:
    ...
    Проверяется форма, а не конкретный номер: привязка к номеру заставляла бы
    править тест при каждом выпуске, и его бы правили не глядя.
    """
    прочитанная = version.released_version()

    assert version.TAG_RE.match(f"v{прочитанная}"), (
        f"версия {прочитанная} не по схеме MAJOR.MINOR.PATCH"
    )
    assert version.check() == [], "литерал в дереве обязан быть законным при теге"


# ── выпуск в подготовке ───────────────────────────────────────────────────


def test_литерал_на_шаг_впереди_тега_законен(репозиторий: Path) -> None:
    """Иначе гейт запрещал бы сам выпуск, ради которого заведён.

    Порядок из `docs/release.md` не переставляется: литерал поднимается
    ПЕРВЫМ шагом, тег ставится ПОСЛЕДНИМ — на уже смерженный коммит. Значит в
    изменении, которое выпуск готовит, литерал обязан быть впереди тега.

    Первая редакция требовала равенства и обнаружилась на первом же выпуске
    после своего появления: та же ловушка, что в #46, этажом ниже.
    """
    (репозиторий / version.VERSION_PATH).write_text(
        '__version__ = "0.2.0"\n', encoding="utf-8"
    )
...
def test_фундаментальный_выпуск_тоже_законен(репозиторий: Path) -> None:
    """`1.0` по схеме — не «следующий после 0.9», а решение о сути."""
    (репозиторий / version.VERSION_PATH).write_text(
        '__version__ = "1.0.0"\n', encoding="utf-8"
    )
...
@pytest.mark.parametrize(
    ("литерал", "почему"),
    [
        ("0.3.0", "номер пропущен: после 0.1 идёт 0.2"),
        ("0.2.1", "PATCH-выпусков не бывает: каждый тег vX.Y.0"),
        ("0.0.9", "движение назад"),
        ("2.0.0", "через MAJOR не перепрыгивают"),
    ],
)
def test_незаконный_литерал_находится(
    репозиторий: Path, литерал: str, почему: str
) -> None:
    """Послабление ради выпуска не должно превратиться в «любой номер сойдёт»."""
    (репозиторий / version.VERSION_PATH).write_text(
        f'__version__ = "{литерал}"\n', encoding="utf-8"
    )

    беды = version.check(репозиторий)

    assert len(беды) == 1, почему
    assert литерал in беды[0]


def test_законные_номера_называются_в_отказе(репозиторий: Path) -> None:
    """Отказ без следующего шага заставляет угадывать."""
    (репозиторий / version.VERSION_PATH).write_text(
        '__version__ = "9.9.9"\n', encoding="utf-8"
    )

    беда = version.check(репозиторий)[0]

    assert "0.1.0" in беда and "0.2.0" in беда and "1.0.0" in беда


def test_следующие_номера_считаются_от_тега() -> None:
    assert version.allowed_next("v1.4.0") == ["1.4.0", "1.5.0", "2.0.0"]
