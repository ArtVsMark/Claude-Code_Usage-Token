...
def _pr(
    *,
    ...
    return {
        ...
        # `ref` кладётся как есть, без `or`: пустая строка — законный
        # случай испорченного события, и подмена её умолчанием спрятала бы
        # ровно ту проверку, ради которой она передаётся.
        "head": {"ref": ref, "repo": {"fork": fork}},
    }
...
def test_незнакомая_форма_меток_не_роняет() -> None:
    ...
    assert len(проблемы) == 3  # нет зоны, нет типа и не видно ветки
...
def test_ветка_не_из_agent_это_отказ() -> None:
    """Умолчание окна — `claude/**`, и оно возвращается при каждом перезапуске.

    За одну серию имя пришлось поправлять трижды: правило, которое приходится
    напоминать трижды, вниманием не держится по определению.
    """
    проблемы = _проблемы(ref="claude/release-12")

    assert len(проблемы) == 1
    assert "claude/release-12" in проблемы[0]
    assert "agent/" in проблемы[0]
    assert "переоткрыть" in проблемы[0], "гейт обязан назвать, что именно делать"


def test_ветка_из_agent_проходит() -> None:
    assert _проблемы(ref="agent/pr-check-6") == []


def test_голая_приставка_не_ветка() -> None:
    """`agent/` без имени — приставка, а не ветка: имя обязано что-то называть."""
    assert len(_проблемы(ref="agent/")) == 1


def test_ветки_нет_в_событии_это_отказ() -> None:
    """Проверка, не нашедшая предмета, обязана сказать об этом, а не молчать.

    Пустое `head.ref` — испорченное событие; принять его за «ветка в порядке»
    значит отключить гейт ровно там, где данные испорчены.
    """
    проблемы = _проблемы(ref="")

    assert len(проблемы) == 1
    assert "не нашёл предмета" in проблемы[0]


def test_у_форка_имя_ветки_не_спрашивается() -> None:
    """У внешнего участника ветка называется как ему удобно.

    Требовать от него нашего соглашения значило бы заворачивать верное
    изменение из-за приставки.
    """
    assert _проблемы(ref="fix/typo", fork=True) == []
