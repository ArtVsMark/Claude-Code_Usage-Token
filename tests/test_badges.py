...
def test_набор_проекта_собирается(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Сборщик на живом дереве проекта: значок обязан получиться.

    Сюда переехала сверка «значок совпадает с источником»: пока он лежал в
    `main`, её делал гейт витрины, читая файл из дерева. Файла там больше нет,
    и проверять надо собранное, а не отсутствующее.

    ЗАМЕР ПОКРЫТИЯ ПОДСТАВЛЕН, а не берётся с диска, и это не удобство. В дереве
    его нет по построению — его делает прогон значков, — и тест, полагающийся на
    случайно лежащий файл, зеленел бы у автора и краснел в прогоне. Ровно так и
    вышло: локально `coverage.json` остался от ручного замера, и первая редакция
    этого теста прошла у автора и упала на всех девяти ячейках матрицы.
    """
    monkeypatch.setattr(facts, "покрытие", lambda root: 87.2)
