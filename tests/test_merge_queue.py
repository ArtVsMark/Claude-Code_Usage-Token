...
def test_эталон_не_содержит_джоба_самой_очереди(площадка: Any) -> None:
    """Дефект, из-за которого конвейер не смержил бы ничего и никогда.

    Эталон брался как check-runs коммита общей ветки. Очередь сама ходит по
    ней и оставляет там свой check-run «подвинуть очередь» — а на pull request
    такой джоб не создаётся: обход на PR не запускается. У любого PR вечно
    недоставало одного имени, вердикт выходил «джобы не созданы», и готовым
    не становился никто.
    ...
    busy, red, эталон = merge_queue.main_state("o/r", "main")

    assert эталон == {"гейты"}
    assert "подвинуть очередь" not in эталон
    assert not busy and not red


def test_эталон_берётся_из_прогона_ci_а_не_из_коммита(площадка: Any) -> None:
    """Спрашивается именно `ci`, а не «все прогоны на ветке».

    «Все прогоны» вернули бы и обход очереди, и любой будущий workflow общей
    ветки — то есть ту же болезнь, только позже и незаметнее.
    """
