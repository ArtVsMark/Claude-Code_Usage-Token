...
class ЗамераНет:
    ...
    Отличать обязательно (039). ``None`` означает «правила вывода нет вовсе»,
    и это отказ: объявленный значок сверялся бы с пустотой. А «замер не
    сделан» — законное состояние дерева: покрытие меряет прогон значков, и
    требовать его файл в каждом локальном прогоне значило бы заворачивать
    верное.

    Сборке значка это не поблажка: ``badges.build`` на этом исходе **падает**,
    потому что писать значок не из чего, и пустой файл на ветке отвечал бы
    вместо живого числа.
    """


#: Производное, лежащее под `.gitignore`. Оставшись от ручного прогона, оно
#: делает гейт зелёным там, где в прогоне файла нет.
СОБРАННОЕ: tuple[str, ...] = (".github/badges", "coverage.json", ".coverage")


def собранное_в_дереве(root: Path) -> list[str]:
    """Производные файлы, оставшиеся от ручного прогона.

    ЗАМЕЧАНИЕ, А НЕ ОТКАЗ, и это не мягкость. Собрать значки или замерить
    покрытие локально — законное действие, и краснеть на нём значило бы
    запрещать проверку руками. Но следующий прогон `preflight` на таком дереве
    отвечает не о том состоянии, которое уедет: гейт витрины видит файл, гейт
    фактов видит замер, а в прогоне их нет.

    За одну серию это случилось ТРИЖДЫ, и каждый раз красное находил не автор,
    а матрица из девяти ячеек — то есть цена промаха платилась полным кругом
    прогона. Отсюда и замечание: назвать вслух то, что молча меняет смысл
    ответа.

    ВТОРЫМ МЕХАНИЗМОМ ЭТО НЕ ЯВЛЯЕТСЯ, хотя рядом и есть отказ. У значка в
    дереве свой предмет и свой тест — `значка версии в дереве нет`: значок в
    общей ветке запрещён сам по себе, независимо от того, чем он там оказался.
    Здесь предмет другой — СОСТОЯНИЕ ДЕРЕВА, на котором вынесены ответы всех
    остальных гейтов, и у замера покрытия отказа нет вовсе: он ничего не
    нарушает, он лишь делает ответ относящимся не к тому состоянию.
    """
    return [имя for имя in СОБРАННОЕ if (root / имя).exists()]


def expected_badge(qid: str, root: Path) -> dict[str, object] | ЗамераНет | None:
    ...
    if qid == "coverage":
        # Замер читается из facts: один источник на значок и на факты.
        доля = facts.покрытие(root)
        if доля is None:
            return ЗамераНет()
        return {
            "schemaVersion": 1,
...
def _check_badge(
    ...
    if isinstance(expected, ЗамераНет):
        # Правило есть, числа под ним в дереве нет: замер делает прогон
        # значков. Молчание здесь честно, а отказ заворачивал бы верное.
        return findings
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    остатки = собранное_в_дереве(ROOT)
    if остатки:
        warned.append(
            "в дереве лежит собранное: "
            + ", ".join(остатки)
            + ". Оно под .gitignore, в прогоне его нет, и ответы гейтов витрины "
            "и фактов сейчас относятся не к тому состоянию, которое уедет"
        )
    ...
    # ЧИСЛО ДОЛГА В ЗАГОЛОВКЕ, а не в чужой задаче раз в сутки. Правило 177
    # требует, чтобы незакрытая работа по правилам называлась первым, что видит
    # проект: `active` без механизма означает «правило объявлено действующим и
    # не действует». Отказом число станет, когда дойдёт до нуля.
    имя_ответа = (
        f"ответ каталогу разрешим (записей {ответ.записей}, "
        f"с адресом {ответ.с_адресом}, ничем не держится {ответ.ничем})"
    )
    if ответ.находки:
    ...
    # слиянии пересобирает, и утверждение «подпись одна» верно или неверно
    # только про то, что уехало. Мелкий клон — воздержание, а не «чисто»:
    # `git log` там пуст не потому, что подписи в порядке.
    подписи = attribution.проверить(ROOT)
    имя_подписей = (
        f"подпись в истории (коммитов {подписи.коммитов}, имён {len(подписи.имена)})"
    )
    if подписи.истории_нет:
        warned.append(
            f"подпись в истории: истории после {attribution.НАЧАЛО[:7]} не видно "
            "— сверять нечего (мелкий клон)"
        )
    elif подписи.находки:
        failed.append((имя_подписей, "\n".join(подписи.находки)))
    else:
        passed.append(имя_подписей)

    # РАСПИСАНИЕ ТРАТИТ КВОТУ ФОНОМ. Прогон, ходящий 48 раз в сутки, выжигает
    # лимит не рывком, и отказ приходит СОСЕДНЕМУ прогону, а не ему. Цену
    # объявляет автор числом, а гейт считает худший час и сверяет с бюджетом;
    # незаявленный прогон по расписанию — отказ.
    расписания = check_schedules.проверить(ROOT)
    имя_расписаний = (
        f"цена расписаний ({расписания.расписаний}; худший час "
        f"{расписания.худший_час} из {расписания.бюджет})"
    )
    if расписания.переписи_нет:
        warned.append(
            "цена расписаний: переписи нет — считать нечего "
            "(её адрес держит ответ каталогу, правило 033)"
        )
    elif расписания.находки:
        failed.append((имя_расписаний, "\n".join(расписания.находки)))
    else:
        passed.append(имя_расписаний)

    # ИСКЛЮЧИТЕЛЬНОЕ УТВЕРЖДЕНИЕ СВЯЗЫВАЕТ ВЕСЬ ДОКУМЕНТ, а не раздел, где
    # написано. Второе такое же о том же предмете не уточняет первое — каждое
    # запрещает второе, и оба перестают действовать. Гейт держит перепись:
    # предмет занят, текст жив, документ существует. Незаписанное утверждение —
    # суждение, и это названо в самом гейте.
    утверждения = exclusive_claims.проверить(ROOT)
    имя_утверждений = (
        f"исключительные утверждения ({утверждения.утверждений} "
        f"в документах {утверждения.документов})"
    )
    if утверждения.переписи_нет:
        warned.append(
            "исключительные утверждения: переписи нет — сверять нечего "
            "(её адрес держит ответ каталогу, правило 181)"
        )
    elif утверждения.находки:
        failed.append((имя_утверждений, "\n".join(утверждения.находки)))
    else:
        passed.append(имя_утверждений)
