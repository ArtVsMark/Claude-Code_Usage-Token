...
#: Зачин ответа `absent`, которым он объявляет себя **утверждением о
#: действительности**, а не решением. Два рода отсутствия смешивать нельзя:
#: «значка нет» — предмет есть, показывать его своим числом не стали, и спорить
#: тут не с чем; «предмета нет» — заявление, которое либо верно, либо нет.
ЗАЧИН_ПРЕДМЕТА = "предмета нет"

#: Вопросы, у которых существование предмета проверяется **механически** — без
#: сети и без догадок о прозе. Значение отвечает, чем предмет подтверждается,
#: или `None`, если подтверждения не видно.
#:
#: Проза в `absent` непроверяема, и гейт её не трогает: он держит подкласс, где
#: утверждение сводится к «тег либо есть в дереве, либо нет». Ровно на этом
#: подклассе витрина и соврала — ответ «проект не выпускался ни разу» пережил
#: два выпуска и был бы отброшен первым же читателем вместе со всем остальным,
#: что витрина говорит (#59).
ПРЕДМЕТ_ПОДТВЕРЖДАЕТСЯ: dict[str, Callable[[Path], str | None]] = {
    "release": lambda root: (
        f"тег выпуска {тег}" if (тег := version.latest_tag(root)) else None
    ),
}
...
def _check_badge(
    ...
    # лежали в `main`, эта проверка стояла ПОСЛЕ выхода по ветке, то есть
    # переезд на ветку молча снял бы её вместе со сверкой файла.
    try:
    ...
    if question.get("branch", "main") != "main":
        ...
        #
        # ЦЕНА НАЗВАНА: сверки значка с деревом тем же прогоном больше нет.
        # Держит свежесть .github/workflows/badges.yml — он собирает значок на
        # каждый толчок в общую ветку.
        return findings
...
def _проверить_предмет(qid: str, absent: str, root: Path) -> list[str]:
    """Сверить с деревом ответ, объявивший себя утверждением о действительности.

    Воздержание при невидимом подтверждении — не поблажка, а тот же третий
    исход, что у счёта версии: мелкий клон облачного окна тегов не видит, и
    «тега нет» там означало бы «не посмотрели». Гейт краснеет только на
    достоверном, поэтому асимметрия здесь намеренная — предмет НАЙДЕН при
    отрицающем ответе это отказ, предмет не найден это молчание.
    """
    подтверждение = ПРЕДМЕТ_ПОДТВЕРЖДАЕТСЯ.get(qid)
    if подтверждение is None:
        return []
    if not absent.strip().lower().startswith(ЗАЧИН_ПРЕДМЕТА):
        # «Значка нет» — решение, а не утверждение: сверять его не с чем.
        return []
    найдено = подтверждение(root)
    if найдено is None:
        return []
    return [
        f"{qid}: ответ утверждает «{ЗАЧИН_ПРЕДМЕТА}», а предмет есть — "
        f"{найдено}. Утверждение о действительности устаревает МОЛЧА: его не "
        "двигает никакой механизм, витрина продолжает отвечать уверенно и "
        "неверно, и первый же читатель сверит ответ с площадкой — а отбросит "
        "вместе с ним всё остальное, что витрина говорит"
    ]
    ...
    for номер, question in enumerate(questions, 1):
        ...
        if absent:
            ...
            if not isinstance(absent, str):
                ...
                findings.extend(_проверить_предмет(qid, absent, root))
            continue
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    имя_контракта = (
        ...
        f"живым числом {контракт.live}, названо пробелов {контракт.named})"
    )
    ...
    # ФАКТЫ ТЕРЯЮТСЯ МОЛЧА, и это их единственный способ соврать. Отсутствие
    # раздела в `facts.json` означает «не измеряли», и витрина соседа честно
    # покажет именно это — вместо числа, которое у нас есть. Матрица, в которую
    # однажды допишут `include`, отняла бы `checks_per_pr` без единого красного.
    потеряно = facts.не_измеренное(ROOT)
    считается = len(facts.ИЗМЕРЯЕМ) - len(потеряно)
    имя_фактов = f"факты о проекте (разделов {считается} из {len(facts.ИЗМЕРЯЕМ)})"
    if потеряно:
        failed.append(
            (
                имя_фактов,
                "\n".join(
                    f"{ключ}: раздел объявлен измеряемым, а посчитать его на "
                    "этом дереве не вышло — в facts.json его не будет, и "
                    "снаружи это неотличимо от «мы такого не меряем»"
                    for ключ in потеряно
                ),
            )
        )
    else:
        passed.append(имя_фактов)
