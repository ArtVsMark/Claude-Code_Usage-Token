...
## Чем закрывается «CI не стартовал»

Обязательной проверкой `PR check`: она зелёная только тогда, когда прошли все
прогоны на голове PR, а состав обязательного читает из дерева самого изменения.
Здесь достаточно потребовать её — и потребовать, чтобы зелены были все прочие
проверки головы.

Эталона имён с общей ветки здесь больше нет. Он ломал любой PR, который состав
проверок **меняет**: имена приходили из прошлого, и вердикт «джобы не созданы»
становился вечным (#46).
...
def check_problems(snapshot: Snapshot) -> tuple[list[str], bool]:
    ...
    Два условия, и второе не заменяет первое. Обязательная проверка `PR check`
    отвечает за полноту: она зелёная, только когда прошли все прогоны на голове
    PR. Остальные проверки смотрятся отдельно, потому что на голове может
    оказаться красное, которого `PR check` не ждал, — сторонний бот, например.
    """
    ...
    обязательная = последние.get(pr_check.SELF_NAME)
    if обязательная is None:
        return (
            [
                f"обязательной проверки «{pr_check.SELF_NAME}» на голове нет. "
                "Она и отвечает за полноту набора, поэтому её отсутствие это "
                "«ещё не стартовало», а не «нечего ждать»"
            ],
            True,
        )
...
def evaluate(snapshot: Snapshot) -> Verdict:
    ...
    return Verdict(
        ...
        [
            ...
            f"включая обязательную «{pr_check.SELF_NAME}»"
        ],
    )
