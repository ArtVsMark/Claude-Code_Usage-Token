...
#: Отказ площадки по её же правилам. Не поломка обхода: состояние PR площадка
#: пересчитывает не мгновенно, и очередь успевает попросить мерж раньше.
MERGE_REFUSED = 405
...
def merge(repo: str, number: int) -> bool:
    ...
    ## Почему 405 не красит прогон

    Замер: обязательная проверка на голове #54 завершилась в 10:09:24, очередь
    прочла состояние и попросила мерж в 10:09:28 — площадка отказала, потому
    что её вычисление ruleset ещё не догнало: «Required status check "PR check"
    is expected». Через минуту тот же PR смержился без единого изменения.

    Обход при этом выходил кодом 2 и красил `main`. Красное из-за секундного
    отставания площадки приучает читать красное как шум — ровно та поломка, от
    которой в проекте заведены правила чтения проверок.

    Поэтому такой отказ печатается предупреждением и оставляет PR следующему
    обходу. Молчаливым он не становится: сообщение площадки печатается
    целиком, и если причина не временная — «нужно одобрение», «ветка
    отстала», — она будет видна в каждом прогоне, а не спрятана за зелёным.
    """
    try:
        gh_rest.request(
            "PUT",
        ...
        print(
            ...
            f"правилам — {exc}. Обход идемпотентен, следующий попробует снова"
        )
        return False
    return True
