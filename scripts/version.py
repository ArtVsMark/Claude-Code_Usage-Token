...
* **выпущенная** — литерал `__version__` в `src/claude_code_usage/__init__.py`.
  ...
  либо на шаг впереди, пока выпуск готовится, — это держит :func:`check`;
...
def _git(*args: str, root: Path = ROOT) -> str | None:
    ...
    try:
            ...
            capture_output=True,
            text=True,
            # БЕЗ ЯВНОЙ КОДИРОВКИ `text=True` берёт кодировку локали, а на
            # windows-раннере это cp1252. Внутренний язык проекта русский, и
            # git отдаёт русские темы коммитов: байт 0x81 (буква «с» в UTF-8)
            # в cp1252 не определён вовсе, поток декодирования падает в своей
            # нити, `stdout` остаётся None — и падает уже вызывающий, с
            # сообщением про NoneType, где искать причину негде.
            #
            # Замер: `гейты · windows-latest · python 3.11` и `3.13` на #62.
            # На ubuntu и macos локаль UTF-8, поэтому промах виден только на
...
def allowed_next(tag: str) -> list[str]:
    """Какие литералы законны при этом теге: нынешний и два следующих.

    Первая редакция требовала равенства литерала и тега — и **запрещала сам
    выпуск**. Порядок из `docs/release.md` не переставляется: литерал
    поднимается ПЕРВЫМ шагом, тег ставится ПОСЛЕДНИМ, на уже смерженный коммит.
    То есть в изменении, которое выпуск и готовит, литерал обязан быть впереди
    тега, а гейт объявлял это расхождением.

    Ловушка та же, что в #46, этажом ниже: проверка делала невозможным ровно
    тот класс изменений, ради которого заведена. Обнаружилась на первом же
    выпуске после её появления.
    """
    major, minor, _ = (int(часть) for часть in TAG_RE.match(tag).groups())  # type: ignore[union-attr]
    return [
        f"{major}.{minor}.0",  # между выпусками
        f"{major}.{minor + 1}.0",  # готовится обычный выпуск
        f"{major + 1}.0.0",  # готовится фундаментальный
    ]


def check(root: Path = ROOT) -> list[str]:
    """Законен ли литерал при нынешнем теге.

    Инвариант схемы — каждый тег `vX.Y.0`, поэтому литерал либо равен тегу,
    либо на один шаг впереди: готовящийся выпуск. Всё остальное — дефект:
    пропущенный номер, ненулевой PATCH, движение назад.
    """
    тег = latest_tag(root)
    if тег is None:
        return []
    выпущенная = released_version(root)
    законные = allowed_next(тег)
    if выпущенная not in законные:
        return [
            f"выпущенная версия {выпущенная} при теге {тег} не законна: "
            f"ожидается {законные[0]} (между выпусками) либо {законные[1]} / "
            f"{законные[2]} (готовится выпуск). Инвариант схемы — каждый тег "
            "vX.Y.0; см. docs/versioning.md"
        ]
    return []
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if аргументы.check:
        ...
        выпущенная = released_version(ROOT)
        состояние = (
            "между выпусками" if выпущенная == тег.lstrip("v") else "ГОТОВИТСЯ ВЫПУСК"
        )
        print(
            f"версия {выпущенная} при теге {тег} — {состояние}; "
            f"с выпуска принято {сколько}"
        )
        return 0
