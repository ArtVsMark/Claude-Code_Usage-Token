...
#: Имя, вводимое циклом или чтением: `for имя in …`, `read -r имя`. Это тоже
#: объявление переменной, и bash споткнётся о него так же — но по началу
#: строки оно не выглядит присваиванием, и проверка на `=` его не видит.
#:
#: Найдено гейтом на собственном коде: `release.yml` писал `for попытка in
#: 1 2 3 4 5`, и первая редакция гейта эту строку пропустила.
_LOOP_VAR = re.compile(r"^\s*(?:for|select)\s+([^\s;]+)\s+in\b")
_READ_VAR = re.compile(r"^\s*(?:while\s+)?read\s+(?:-\w+\s+)*([^\s;|<]+)")
...
def check_text(text: str, path: str) -> list[Finding]:
    ...
    for номер, строка in enumerate(lines):
        ...
        if номер in в_run:
            фрагмент = в_run[номер]
            совпадение = _ASSIGN.match(фрагмент)
            ...
            for правило, вид in ((_LOOP_VAR, "цикла"), (_READ_VAR, "чтения")):
                объявление = правило.match(фрагмент)
                if объявление and _NON_ASCII.search(объявление.group(1)):
                    findings.append(
                        Finding(
                            path,
                            номер + 1,
                            f"переменная {вид} `{объявление.group(1)}` — bash "
                            "требует ASCII-идентификатор и на таком имени "
                            "споткнётся",
                        )
                    )
