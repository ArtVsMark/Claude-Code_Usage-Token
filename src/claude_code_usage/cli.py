...
def rows_from_transcripts(root: Path, *, ts: str) -> tuple[list[dict[str, Any]], str]:
    ...
    # ОХВАТ ПЕЧАТАЕТ САМ `Coverage`, а не эта функция. Первая редакция строила
    # строку здесь заново — и «забыла» повторы ответа: человек видел «строк с
    # расходом 648» и не знал, что ещё 570 отброшено дедупликацией, то есть
    # 47% строк с `usage`. Число, ради которого дедупликация и заведена,
    # оказалось скрыто ровно от того, кому оно нужно.
    #
    # Тест при этом был зелёным: он проверял `Coverage.__str__`, а не вывод
    # команды. Второй источник одной величины разошёлся с первым при первой же
    # правке — как и обещает правило 022.
    return строки, f"транскрипты: сессий {len(строки)}, {охват}"
